...
class UserActionLogger:
    ...
    @staticmethod
    def log_user_action(action: str, user_id: str, details: dict = None, request_id: str = None):
        """ユーザー操作をログ記録"""
        log_data = {
            "event": "user_action",
            "action": action,
    ...
    @staticmethod
    def log_data_access(
        resource: str, user_id: str, operation: str, resource_id: str = None, request_id: str = None
    ):
        """データアクセスの追跡"""
        log_data = {
            "event": "data_access",
...
def log_authentication(user_id: str, success: bool, ip_address: str, request_id: str = None):
    ...
    if success:
        user_logger.log_security_event(
            "login_success", user_id, {"ip_address": ip_address}, "info", request_id
        )
    else:
        user_logger.log_security_event(
            "login_failure", user_id or "unknown", {"ip_address": ip_address}, "warning", request_id
        )
...
def log_voice_transcription(user_id: str, child_id: str, file_size: int, request_id: str = None):
    ...
    user_logger.log_user_action(
        "voice_transcription",
        user_id,
        {"child_id": child_id, "file_size_bytes": file_size, "feature": "speech_recognition"},
        request_id,
    )


def log_child_data_access(
    user_id: str, operation: str, child_id: str = None, request_id: str = None
):
    """子どもデータへのアクセスログ"""
    user_logger.log_data_access("children_data", user_id, operation, child_id, request_id)
