...
async def generate_feedback_for_challenge(challenge_id: str, db: Session = Depends(get_db)):
    ...
    if child and child.birthdate:
        today = date.today()
        child_age = today.year - child.birthdate.year - ((today.month, today.day) < (child.birthdate.month, child.birthdate.day))
...
async def auto_analyze_challenges(db: Session = Depends(get_db)):
    ...
    try:
        # commentが空のチャレンジを取得
        unanalyzed_challenges = db.query(Challenge).filter(
...
async def get_analysis_status(db: Session = Depends(get_db)):
    ...
    total_with_transcript = db.query(Challenge).filter(
        Challenge.transcript.isnot(None),
        Challenge.transcript != ""
    ).count()
    
    analyzed = db.query(Challenge).filter(
        Challenge.transcript.isnot(None),
        Challenge.transcript != "",
    ...
    return {
        ...
        "analysis_rate": round((analyzed / total_with_transcript * 100), 1) if total_with_transcript > 0 else 0
    }