...
2) この子の「英語チャレンジ」を以下の観点で温かく評価（約100文字）:
   🌟【勇気ポイント】外国人に話しかけた勇気、英語で伝えようとした挑戦心
   💫【成長の芽】単語一つでも英語を使えた、コミュニケーションが成立した
   🎯【次への期待】この経験が次の挑戦への自信になる
   【重要】完璧でなくても、話しかけた勇気と挑戦する気持ちが最も価値がある
3) 会話文脈に沿った簡単な英語フレーズを1つ提案（年齢: {child_age if child_age is not None else "不明"}）

出力は必ず次のJSONだけ:
{{
  "child_utterances": ["子どもと推定した発話1", "発話2"],
  "feedback_short": "🌟💫🎯の観点を含む約100文字の温かい応援コメント",
  "phrase_suggestion": {{ "en": "Thank you", "ja": "ありがとう" }},
